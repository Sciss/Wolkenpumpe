...
class NuagesActions[S <: Sys[S]](nuages: NuagesPanel[S]) {
  ...
  //  def createProc(genF: ProcFactory, diffF: ProcFactory, pt: Point2D)(implicit txn: ProcTxn): Unit = {
  //    // import DSL._
  //    val gen = genF.make
  //    val diff = diffF.make
  //    gen ~> diff
  ...
  //  def createFilter(out: ProcAudioOutput, in: ProcAudioInput, fltF: ProcFactory, pt: Point2D)
  //                  (implicit txn: ProcTxn): Unit = {
  //    // import DSL._
