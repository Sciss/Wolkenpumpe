...
object NuagesPanel {
  ...
  val soloAmpSpec         = ParamSpec(0.10, 10.0, ExpWarp) -> 0.5
}

class NuagesPanel[S <: Sys[S]](val config: NuagesConfig)(implicit cursor: stm.Cursor[S])
  extends JPanel /* with ProcFactoryProvider[S] */ {
  panel =>
...
//    }
  ...
  private def efficientOuts(sig: GE, chans: IIdxSeq[Int]): Unit = {
