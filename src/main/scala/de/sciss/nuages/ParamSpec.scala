...
import de.sciss.lucre.expr.{Expr => Ex, Double => DoubleEx, String => StringEx, ExprType1, Type}
...
object ParamSpec {
  ...
  object Expr extends ExprLikeType[ParamSpec, ParamSpec.Expr] with TypeImpl1[ParamSpec.Expr] {
    ...
    def readConst[S <: Sys[S]](in: DataInput): Repr[S] with Ex.Const[S, ParamSpec] = {
      val cookie = in.readByte()
      if (cookie != 3) sys.error(s"Unexpected cookie $cookie")
      newConst[S](readValue(in))
    }

    def newConst[S <: Sys[S]](value: ParamSpec): Repr[S] with Ex.Const[S, ParamSpec] = Const[S](value)

    def read[S <: Sys[S]](in: DataInput, access: S#Acc)(implicit tx: S#Tx): Repr[S] = serializer[S].read(in, access)

    def apply[S <: Sys[S]](lo: Ex[S, Double], hi: Ex[S, Double], warp: Ex[S, Warp],
                           step: Ex[S, Double], unit: Ex[S, String])(implicit tx: S#Tx): Repr[S] =
      (lo, hi, warp, step, unit) match {
        case (Ex.Const(loC), Ex.Const(hiC), Ex.Const(warpC), Ex.Const(stepC), Ex.Const(unitC)) =>
    ...
    private[this] final case class Const[S <: Sys[S]](constValue: ParamSpec)
      ...
      def lo  (implicit tx: S#Tx): Ex[S, Double] = DoubleEx .newConst(constValue.lo  )
      def hi  (implicit tx: S#Tx): Ex[S, Double] = DoubleEx .newConst(constValue.hi  )
      def warp(implicit tx: S#Tx): Ex[S, Warp  ] = Warp.Expr.newConst(constValue.warp)
      def step(implicit tx: S#Tx): Ex[S, Double] = DoubleEx .newConst(constValue.step)
      def unit(implicit tx: S#Tx): Ex[S, String] = StringEx .newConst(constValue.unit)

      protected def writeData(out: DataOutput): Unit = writeValue(constValue, out)
    }

    private[this] object Apply extends Type.Extension1[Repr] {
    ...
    private[this] final class Apply[S <: Sys[S]](protected val targets: evt.Targets[S],
                                                 loEx: Ex[S, Double], hiEx: Ex[S, Double],
                                                 warpEx: Ex[S, Warp], stepEx: Ex[S, Double],
                                                 unitEx: Ex[S, String])
      extends Repr[S]
      ...
      def lo  (implicit tx: S#Tx): Ex[S, Double] = loEx
      def hi  (implicit tx: S#Tx): Ex[S, Double] = hiEx
      def warp(implicit tx: S#Tx): Ex[S, Warp  ] = warpEx
      def step(implicit tx: S#Tx): Ex[S, Double] = stepEx
      def unit(implicit tx: S#Tx): Ex[S, String] = unitEx
    ...
    private[this] final class Var[S <: Sys[S]](val targets: evt.Targets[S], val ref: S#Var[Repr[S]])
      ...
      def lo  (implicit tx: S#Tx): Ex[S, Double] = mkTuple1(this, DoubleExtensions.Lo)
      def hi  (implicit tx: S#Tx): Ex[S, Double] = mkTuple1(this, DoubleExtensions.Hi)
      def warp(implicit tx: S#Tx): Ex[S, Warp  ] = mkTuple1(this, WarpExtensions.Warp)
      def step(implicit tx: S#Tx): Ex[S, Double] = mkTuple1(this, DoubleExtensions.Step)
      def unit(implicit tx: S#Tx): Ex[S, String] = mkTuple1(this, StringExtensions.Unit)
    ...
    private[this] object DoubleExtensions
      extends Type.Extension1[({type Repr[~ <: Sys[~]] = Ex[~, Double]})#Repr] {
      ...
      def readExtension[S <: Sys[S]](opID: Int, in: DataInput, access: S#Acc, targets: evt.Targets[S])
                                    (implicit tx: S#Tx): Ex.Node[S, Double] = {
    ...
    private[this] object StringExtensions
      extends Type.Extension1[({type Repr[~ <: Sys[~]] = Ex[~, String]})#Repr] {
      ...
      def readExtension[S <: Sys[S]](opID: Int, in: DataInput, access: S#Acc, targets: evt.Targets[S])
                                    (implicit tx: S#Tx): Ex.Node[S, String] = {
    ...
    private[this] object WarpExtensions
      extends Type.Extension1[({type Repr[~ <: Sys[~]] = Ex[~, nuages.Warp]})#Repr] {
      ...
      def readExtension[S <: Sys[S]](opID: Int, in: DataInput, access: S#Acc, targets: evt.Targets[S])
                                    (implicit tx: S#Tx): Ex.Node[S, nuages.Warp] = {
    ...
    private final class Tuple1[S <: Sys[S], A](val op: Tuple1Op[A], val targets: evt.Targets[S], val _1: Repr[S])
      extends Ex.Node[S, A]
      with evt.impl.StandaloneLike[S, Change[A], Ex[S, A]]
      ...
      def changed: Event[S, Change[A], Ex[S, A]] = this
    }

    private[this] abstract class Tuple1Op[A] {
      def id: Int
      ...
      final def unapply[S <: Sys[S]](ex: Ex[S, A])(implicit tx: S#Tx): Option[Repr[S]] = ex match {
  ...
  trait Expr[S <: Sys[S]] extends Ex[S, ParamSpec] {
    def lo  (implicit tx: S#Tx): Ex[S, Double]
    def hi  (implicit tx: S#Tx): Ex[S, Double]
    def warp(implicit tx: S#Tx): Ex[S, Warp  ]
    def step(implicit tx: S#Tx): Ex[S, Double]
    def unit(implicit tx: S#Tx): Ex[S, String]
  }
