...
import de.sciss.lucre.expr.{Double => DoubleEx, String => StringEx, ExprType1, Type, Expr}
...
object ParamSpecImpl {
  object ExprImpl
    extends ExprLikeType[ParamSpec, ParamSpec.Expr]
    with TypeImpl1[ParamSpec.Expr]
    with ParamSpec.ExprCompanion {
    ...
    private[this] final class Ser[S <: Sys[S]] extends evt.EventLikeSerializer[S, Repr[S]] {
      def readConstant(in: DataInput)(implicit tx: S#Tx): Repr[S] = ExprImpl.readConst(in)
    ...
    private[this] final class Apply[S <: Sys[S]](protected val targets: evt.Targets[S],
      ...
      def reader: evt.Reader[S, Repr[S]] = ExprImpl.serializer[S]
