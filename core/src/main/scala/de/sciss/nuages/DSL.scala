...
class DSL[S <: stm.Sys[S]] protected() {
  // val imp = ExprImplicits[S]
  import proc.Implicits._

  private[nuages] val current = TxnLocal[Proc[S]]()

  protected def currentProc(implicit tx: InTxn): Proc[S] = current()
  ...
  def pAudioIn(key: String, numChannels: Int = -1)(implicit tx: S#Tx): GE = {
...
//    obj.inputs.add(key)
    ...
    val paramObj: Obj[S] = if (numChannels < 0) {
      DoubleObj   .newVar(DoubleObj   .newConst[S](0.0))
    } else {
      DoubleVector.newVar(DoubleVector.newConst[S](Vector.fill(numChannels)(0.0)))
    }
